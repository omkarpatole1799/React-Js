	...
	return async (dispatch) => {
		const fetchData = async () => {
			const response = await fetch(
				"https://cart-data-01-default-rtdb.firebaseio.com/cart-data.json"
			);
		...
		try {
			...
			dispatch(
				cartItemAction.replaceCartData({
...
export const sendData = (cartData) => {
	return async (dispatch) => {
		dispatch(UIActions.showNotification(notification.sending));
		...
		try {
			await sendData();
			dispatch(UIActions.showNotification(notification.success));
		} catch (error) {
			dispatch(UIActions.showNotification(notification.error));
		}
	};
